...
app.controller('ModalReformasLineasItemController', [ "$scope","$rootScope","ID","unidadID","unidadcodigo","unidadnombre","editar","noeditar","$uibModalInstance","SweetAlert","$filter", "ngTableParams","reformasFactory",
...
//            	  //delete tObj.subitem;
//            	}
            	for ( var index in $scope.listarItems) {
					if (String($scope.listarItems[index].id) == $scope.objeto.item) {
						$uibModalInstance.close($scope.listarItems[index]);
					}
				}
            }
        },
